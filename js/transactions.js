...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const transactionsModule = {
    ...
    async deleteTransaction(transactionId) {
      try {
        const { error } = await supabase.from("Transactions").delete().eq("TransactionID", transactionId)
    ...
    setupFilters() {
      ...
      filterForm.addEventListener("submit", (e) => {
        ...
        if (assetFilter) {
          filteredTransactions = filteredTransactions.filter((t) => t.AssetID === Number.parseInt(assetFilter))
        }
