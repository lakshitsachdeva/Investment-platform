document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (portfolioSummaryElement) {
      ...
      if (portfolios && portfolios.length > 0) {
        const totalValue = portfolios.reduce((sum, portfolio) => sum + parseFloat(portfolio.PortfolioValue), 0);
        ...
        portfolios.forEach(portfolio => {
          portfolioHTML += `
            <div class="portfolio-item">
              ...
              <span class="value">$${parseFloat(portfolio.PortfolioValue).toFixed(2)}</span>
            </div>
          `;
        });
