document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    if (portfoliosListElement) {
      const { data: portfolios } = await supabase.from("Portfolios").select("*").eq("UserID", user.id)
          ...
          portfoliosHTML += `
            <div class="portfolio-card" data-id="${portfolio.PortfolioID}">
              <div class="portfolio-header">
                ...
                <div class="portfolio-value">$${Number.parseFloat(portfolio.PortfolioValue).toFixed(2)}</div>
              </div>
    ...
    async function deletePortfolio(e) {
      if (confirm("Are you sure you want to delete this portfolio?")) {
        ...
        try {
          const { error } = await supabase.from("Portfolios").delete().eq("PortfolioID", portfolioId)
